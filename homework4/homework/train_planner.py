...
def make_loader(split_path: str, transform_pipeline: str, batch_size: int, num_workers: int, shuffle: bool):
    ...
    return DataLoader(ds, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=True, collate_fn=collate_batch)
...
def train_one_epoch(model, loader, optimizer, device):
    ...
    for batch in loader:
        ...
        # Figure out which inputs the model expects by name
        if model.__class__.__name__ == 'CNNPlanner':
            images = batch['image'].to(device, non_blocking=True)  # (B, 3, 96, 128)
            preds = model(images)
        else:
...
def train(
    ...
    Returns a dict of final val stats.
    """
...
def main():
    ...
    p.add_argument('--model', choices=['mlp_planner','transformer_planner','cnn_planner'], required=True)
    ...
    p.add_argument('--transform', type=str, choices=['default','state_only'], default=None, help='Override transform pipeline')
    p.add_argument('--save', action='store_true', help='Save best checkpoint')
    args = p.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = build_model(args.model).to(device)

    # Choose transform pipeline: planners that use state should use 'state_only'
